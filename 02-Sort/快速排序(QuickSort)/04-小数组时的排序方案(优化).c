...
/*
程序背景：
	当数据较小的时候，我们可以不要使用快速排序而使用直接插入排序更快。我们增加了一个判断，
	当high-low不大于某个常数时（有资料认为7比较合适，也有认为50更合理，实际应用可适当调整），
	就用直接插人排序，这样就能保证最大化地利用两种排序的优势来完成排序工作。
	
程序功能：
	当断数组时直接使用直接插入排序，而不适用快排，以此来提升排序速度。
程序环境：
	VS2019 x86 debug
*/
...
void QuickSort(int a[], int low, int high)
{
	...
	{
		pivot = partition(a, low, high); //并算出枢轴值pivot
